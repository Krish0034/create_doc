...
class UserNameBloc extends Bloc<UserNameEvent, UserNameState> {
  ...
  UserNameBloc(this._userNameAccessServices) : super(UserNameState.initial()) {
    on<UserNameEvent>((event, emit) async{
     await event.when(
        userExist: (String userName, AuthType authType) => _userExist(userName, authType, emit),
      );
    });
  }

  Future<dynamic> _userExist(String userName, AuthType authType, Emitter<UserNameState> emit) async{
    final result = await _userNameAccessServices.userExist(userName: userName, authType: authType);
    Logger.data("user name is exist or not $result");
    // await emit(
    //   result.fold(
    //           (ErrorData errorData) => state.copyWith(userName: false,userData: none()),
    //           (UserData userData) => state.copyWith(userName: true,userData: optionOf(userData))
    //   )
    // );
    await result.fold(
